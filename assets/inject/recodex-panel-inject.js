...
(() => {
  ...
  const I18N = {
    tw: {
      ...
      "ReCodex 桥未就绪": "ReCodex 橋未就緒", "设置没有生效,请重试": "設定沒有生效,請重試", "数据可能不是最新的": "資料可能不是最新的", "配置没有保存成功,请重试": "設定沒有儲存成功,請重試", "ReCodex 桥未响应": "ReCodex 橋沒有回應",
    ...
    ru: {
      ...
      "ReCodex 桥未就绪": "Мост ReCodex не готов", "设置没有生效,请重试": "Настройка не применилась, попробуйте ещё раз", "数据可能不是最新的": "Данные могут быть устаревшими", "配置没有保存成功,请重试": "Настройки не сохранились, попробуйте ещё раз", "ReCodex 桥未响应": "Мост ReCodex не отвечает",
  ...
  async function render() {
    ...
    // 用户看到 4% 会以为真的还剩 96%。
    if (res.status === "stale") {
      const why = [d.account_error, d.gateway_error].filter(Boolean).join(" / ");
      html += `<div class="rcx-err" style="font-size:12px;margin-bottom:6px">${
        t("数据可能不是最新的")}${why ? `:${esc(why)}` : ""}</div>`;
    }
    const acc = d.account || {};
    const u = d.usage || {};
