...
/// 上游 issue #1965:PureApi 会把 config.toml 里的 `experimental_bearer_token` 移除,
/// 只留 auth.json 一个落点。旧实现要求 auth.json 为空才回填,于是退出 ChatGPT 登录后
/// 残留的 `{"last_refresh": ...}` 会把回填挡掉 —— key 两边都没有,Codex 只能读
/// OPENAI_API_KEY 环境变量,上游返回 401。
#[test]
