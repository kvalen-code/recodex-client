...
fn injection_script_keeps_bundled_marketplace_name_for_default_filter() {
    ...
    assert!(script.contains("if (name === \"openai-bundled\") return \"OpenAI插件1(ReCodex)\""));
}
...
fn injection_script_expands_api_key_plugin_marketplace_requests() {
    ...
    assert!(script.contains("if (name === \"openai-curated\") return \"OpenAI插件2(ReCodex)\""));
    assert!(
        script.contains("if (name === \"openai-primary-runtime\") return \"OpenAI插件3(ReCodex)\"")
    );
    assert!(script.contains("restored === \"openai-api-curated\""));
    assert!(script.contains("restored === \"openai-curated-remote\""));
    assert!(
        script.contains("if (name === \"openai-curated-remote\") return \"OpenAI插件5(ReCodex)\"")
    );
    assert!(script.contains(
        "if (name === \"codex-plus-openai-curated-remote\") return \"openai-curated-remote\""
    ));
    assert!(script.contains("OpenAI插件1(ReCodex)"));
    assert!(script.contains("OpenAI插件2(ReCodex)"));
    assert!(script.contains("OpenAI插件3(ReCodex)"));
